...
import 'package:flutter_app/components/pages/talent/talent_register.dart';

class TalentLogin extends StatefulWidget {
  TalentLogin({Key key}) : super(key: key);

  @override
  _TalentLoginState createState() => _TalentLoginState();
}

class _TalentLoginState extends State<TalentLogin> {
  Map agentUserInfo = {};
  int _activeBtn = 0; // 顶部切换按钮的索引
  int _activeTag = 0; // 有还是没有切换的索引
  List<Widget> _createChip() {
    return Constants.talentUserList.map((value) {
      return GestureDetector(
        ...
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => TalentRegister(step: value["step"]),
            ),
          );
        },
      );
            ...
            children: <Widget>[
              Container(
                ...
                child: Column(
                  children: <Widget>[
                    SizedBox(height: 15),
                    Text('寻找   面试   匹配   分析', style: TextStyle(color: Constants.COLOR_1FB3C4),),
                    Center(
                      child: Text(
                        "专业顾问帮您办",
                        style: TextStyle(
